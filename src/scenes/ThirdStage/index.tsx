...
export const ThirdStage = () => {
  ...
  const isOpenDoorLeft = useRef(true);
  const isOpenDoorRight = useRef(false);
  ...
  // const actionsOpenLeft = actions[OPEN_CLIP_1];
  // const actionsCloseLeft = actions[CLOSE_CLIP_1];

  //     const actionsOpenRight = actions[OPEN_CLIP_2];
  //   const actionsCloseRight = actions[CLOSE_CLIP_2];
  ...
  //   if (isOpenDoorLeft.current) {
  //     actionsClose.stop();
  //     playAction(actionsOpen);
  //   } else {
  //     actionsOpen.stop();
  //     playAction(actionsClose);
  //   }
  // }, [actions]);
  ...
  useFrame(() => {
    ...
    const wasOpenLeft = isOpenDoorLeft.current;
    const wasOpenRight = isOpenDoorRight.current;

    isOpenDoorLeft.current = dotProductLeft > 0;
    isOpenDoorRight.current = dotProductRight < 0;

    // Триггерим анимацию только при изменении состояния
    if (wasOpenLeft !== isOpenDoorLeft.current) {
      const actionsOpen = actions[OPEN_CLIP_1];
      const actionsClose = actions[CLOSE_CLIP_1];
      if (isOpenDoorLeft.current) {
        actionsClose?.stop();
    ...
    if (wasOpenRight !== isOpenDoorRight.current) {
      const actionsOpen = actions[OPEN_CLIP_2];
      const actionsClose = actions[CLOSE_CLIP_2];
      if (isOpenDoorRight.current) {
        actionsClose?.stop();
